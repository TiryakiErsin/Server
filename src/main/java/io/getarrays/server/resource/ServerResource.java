...
import static io.getarrays.server.model.Response.*;
...
public class ServerResource {  // Controller

    private final ServerServiceImpl serverService;

    @GetMapping("/list")
    public ResponseEntity<Response> gerServers(){
    ...
    public ResponseEntity<Response> updateServer(@RequestBody @Valid Server server)  {
        ...
        return ResponseEntity.ok(
                Response.builder()
                        .timeStamp(now())
                        .data(of("updated", serverService.update(server)))
                        .message("Server deleted")
                        .status(OK)
                        .statusCode(OK.value())
                        .build()
    ...
    public byte[] getServerImage(@PathVariable("fileName") String fileName) throws IOException {
        ...
        return Files.readAllBytes(Paths.get(System.getProperty("user.home") + "Pictures/Camera Roll" +fileName));
